...
function [p11,PSNR,mssim,i,fun1] = Precondition_TOS_Nonconvex_MC_ATV(x_true,b,sigma,tau,tol,iter,a1,mu,mu1,alpha)
...
% g = @(x) A_adj(A_dir(x) - b);
 ...
 g = @(x) x - b - alpha*mu*opDadj(opD(x)-max(abs(opD(x))-1/alpha,0).*sign(opD(x)));
 ...
 while i <= iter
   ...
   %     derror = norm(p11(:)-x_true(:))/norm(x_true(:)); 
    %     snr(i) = 20*log10(norm(x_true(:))/norm(x_true(:)-p11(:)));
       ...
       err = max(norm(x1_update(:)-x1(:))/norm(x1(:)),norm(x2_update(:)-x2(:))/norm(x2(:)));
        
     v_mcp = opD(p11);
    fun_mcp = prox_MCP(v_mcp(:),1,1/alpha,1);
        fun1(i) = 0.5*norm(p11(:)-b(:))^2 + mu*sum(fun_mcp) + mu1*nuclear_norm(p11);
